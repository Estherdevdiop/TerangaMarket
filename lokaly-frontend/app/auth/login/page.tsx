...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Mmm, email ou mot de passe pas bons. Réessaie ! 😊')
...
<Link href="/" className="font-display text-3xl font-bold text-[#e8720a]">TerangaMarket</Link>
          <p className="text-sm text-[#8b6030] mt-1">Retourne chez toi ! 👋</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-[#1a1208] mb-1.5">Ton mot de passe</label>
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full py-3 disabled:opacity-60">
            {loading ? 'C\'est parti...' : 'Me connecter ! 🚀'}
          </button>
        </form>
        <p className="text-center text-sm text-[#8b6030]">
          Nouveau chez nous ? <Link href="/auth/register" className="text-[#e8720a] font-semibold hover:underline">Inscris-toi !</Link>
        </p>
      </div>
    </div>
  )
